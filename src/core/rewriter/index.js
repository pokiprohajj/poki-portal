...
function rewriteHtml(html, sourcePath) {
  ...
  if ($('head').length) {
    ...
    if (!$('meta[name="robots"]').length) {
      ...
      // Check full URL including query for randomgame (sourcePath does not include query)
      const fullUrl = (typeof req !== 'undefined' && req.url) ? req.url : sourcePath;
      const isRandomGame = fullUrl && fullUrl.includes('randomgame');
